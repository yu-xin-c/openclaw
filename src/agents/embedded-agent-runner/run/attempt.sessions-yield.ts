...
import {
  ...
  waitForAttemptHandoffAbortSettle,
...
export const createYieldAbortedResponse = createAttemptHandoffAbortedResponse;
...
export const SESSIONS_YIELD_ABORT_REASON = createAttemptHandoffAbortReason("sessions_yield");
...
export function isSessionsYieldAbortError(err: unknown): boolean {
  return isAttemptHandoffAbortError(err, "sessions_yield");
}

export function isSessionsYieldAbortReason(reason: unknown): boolean {
  return isAttemptHandoffAbortReason(reason) && reason.code === "sessions_yield";
}
